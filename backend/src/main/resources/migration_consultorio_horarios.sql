...
COMMENT ON COLUMN consultorio.hora_apertura_default IS 'Hora de apertura por defecto del consultorio (formato HH:mm)';
COMMENT ON COLUMN consultorio.hora_cierre_default IS 'Hora de cierre por defecto del consultorio (formato HH:mm)';
COMMENT ON TABLE horario_consultorio IS 'Horarios específicos por día de la semana para cada consultorio';
COMMENT ON COLUMN horario_consultorio.dia_semana IS 'Día de la semana (LUNES, MARTES, MIERCOLES, JUEVES, VIERNES, SABADO, DOMINGO)';
COMMENT ON COLUMN horario_consultorio.activo IS 'Indica si el consultorio atiende en este día';
