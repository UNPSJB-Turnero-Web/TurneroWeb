...
export class TurnoAdvancedSearchComponent implements OnInit {
  ...
  tiposVista = [
    ...
    {
      value: "reporte-dia-consultorio",
      label: "Reporte: Turnos por Día y Consultorio",
      icon: "fas fa-calendar-day",
    },
    {
      value: "reporte-especialidad-medico",
      label: "Reporte: Turnos por Especialidad y Médico",
      icon: "fas fa-user-md",
    },
    {
      value: "reporte-cancelaciones",
      label: "Reporte: Cancelaciones y Reprogramaciones",
  ...
  search(): void {
    ...
    this.turnoService.searchWithFilters(cleanFilter).subscribe({
      next: (response: DataPackage<any>) => {
        ...
        const isSuccessful =
          response.status === 1 || (response as any).status_code === 200;
  ...
  changeState(): void {
    ...
    if (
      this.changeStateForm.nuevoEstado === "CANCELADO" &&
    ...
    if (
      this.changeStateForm.motivo &&
      this.changeStateForm.motivo.trim().length < 5
    ) {
      ...
      .subscribe({
        next: (response) => {
          ...
          if (isSuccessful) {
            ...
            alert(
              "Error al cambiar estado: " + (response as any).status_text ||
                "Error desconocido"
            );
          }
          this.loading = false;
    ...
    this.turnoService.verifyAuditIntegrity(turnoId).subscribe({
      next: (response: DataPackage<{ isValid: boolean }>) => {
        ...
        if (isSuccessful) {
          ...
          alert(
            isValid
              ? "El historial de auditoría es íntegro"
    ...
    this.turnoService.exportToCSVDownload(exportFilter).subscribe({
      next: (blob: Blob) => {
        ...
        if (blob.type === "text/html" || blob.size < 50) {
          ...
          alert(
            "Error: El servidor devolvió HTML en lugar de CSV. Verifique el endpoint del backend."
          );
          this.exportLoading = false;
          return;
        }
        ...
        this.downloadFile(
          blob,
          `turnos_busqueda_${new Date().toISOString().split("T")[0]}.csv`,
          "text/csv"
        );
        this.exportLoading = false;
      },
  ...
  private exportReportCSV(): void {
    if (!this.hasReportData()) {
      alert(
        "No hay datos de reporte para exportar. Genere un reporte primero."
      );
      return;
    }
  ...
  private generateReportCSVData(): string {
    ...
    switch (this.tipoVista) {
      case "reporte-dia-consultorio":
        csvContent =
          "Fecha,Consultorio,Total Turnos,Atendidos,Cancelados,Reprogramados\n";
        this.reportData.turnosPorDiaConsultorio.forEach((item: any) => {
          csvContent += `${item.fecha || ""},${item.consultorio || ""},${
            item.totalTurnos || 0
          },${item.atendidos || 0},${item.cancelados || 0},${
            item.reprogramados || 0
          }\n`;
        });
        break;
      ...
      case "reporte-especialidad-medico":
        csvContent =
          "Especialidad,Médico,Total Turnos,Atendidos,Cancelados,Reprogramados\n";
        this.reportData.turnosPorEspecialidadMedico.forEach((item: any) => {
          csvContent += `${item.especialidad || ""},${item.medico || ""},${
            item.totalTurnos || 0
          },${item.atendidos || 0},${item.cancelados || 0},${
            item.reprogramados || 0
          }\n`;
      ...
      case "reporte-cancelaciones":
        csvContent =
          "Fecha,Médico,Paciente,Tipo Movimiento,Motivo,Estado Anterior,Estado Nuevo\n";
        this.reportData.cancelacionesReprogramaciones.forEach((item: any) => {
          csvContent += `${item.fecha || ""},${item.medico || ""},${
            item.paciente || ""
  ...
  private handlePDFResponse(blob: Blob, tipo: string = "busqueda"): void {
    ...
    if (
      ...
      console.error(
        "Error: El archivo PDF no tiene el tipo correcto. Tipo recibido:",
        blob.type
      );
      ...
      alert(
        'Error: El servidor devolvió contenido de tipo "' +
          blob.type +
          '" en lugar de PDF. Verifique el endpoint del backend.'
      );
      this.exportLoading = false;
      return;
    }
  ...
  private handlePDFError(error: any): void {
    ...
    if (error.status === 404) {
      errorMessage +=
        "Endpoint no encontrado (404). Verifique que el backend tenga implementado rest/turno/export/pdf";
    } else if (error.status === 500) {
      errorMessage +=
        "Error interno del servidor (500). Revise los logs del backend.";
    } else if (error.status === 405) {
      errorMessage +=
        "Método no permitido (405). El backend no acepta POST ni GET para este endpoint.";
    } else {
  ...
  generateTurnosPorDiaConsultorio(): void {
    ...
    if (
      this.filter.nombreEspecialidad &&
      this.filter.nombreEspecialidad.trim()
    ) {
      ...
      error: (error) => {
        ...
        alert(
          "Error al generar el reporte: " +
            (error.error?.status_text || error.message || "Error desconocido")
        );
        this.reportData.turnosPorDiaConsultorio = [];
        this.reportesLoading = false;
      },
      ...
      next: (response: DataPackage<any>) => {
        ...
        if (isSuccessful) {
          ...
          this.reportData.cancelacionesReprogramaciones =
            this.processCancelacionesReprogramaciones(filtered);

          console.log(
            "Reporte 3 - Datos procesados:",
            this.reportData.cancelacionesReprogramaciones.length
          );
        } else {
  ...
  private groupTurnosByDateAndConsultorio(turnos: Turno[]): any[] {
    ...
    turnos.forEach((turno) => {
      ...
      const medico = `${turno.staffMedicoNombre || "Sin nombre"} ${
        turno.staffMedicoApellido || "Sin apellido"
      }`.trim();
  ...
  private processCancelacionesReprogramaciones(turnos: Turno[]): any[] {
    return turnos
      .map((turno) => {
        // Manejar casos donde faltan datos
        const paciente = `${turno.nombrePaciente || "Sin nombre"} ${
          turno.apellidoPaciente || "Sin apellido"
        }`.trim();
        const medico = `${turno.staffMedicoNombre || "Sin nombre"} ${
          turno.staffMedicoApellido || "Sin apellido"
        }`.trim();
  ...
  exportReportToCSV(): void {
    ...
    const headerInfo =
      `"Reporte de Auditoría de Turnos"\n` +
      `"Período: ${this.formatDate(
        this.filter.fechaDesde || ""
      )} - ${this.formatDate(this.filter.fechaHasta || "")}"\n` +
    ...
    switch (this.tipoVista) {
      case "reporte-dia-consultorio":
        csvContent =
          headerInfo +
          '"Turnos por Día y Consultorio"\n' +
          this.generateCSVForTurnosDiaConsultorio();
        break;
      case "reporte-especialidad-medico":
        csvContent =
          headerInfo +
          '"Turnos por Especialidad y Médico"\n' +
  ...
  private async createPDFFromElement(
    element: HTMLElement,
    fileName: string
  ): Promise<void> {
    try {
  ...
  private generateReportFileName(extension: string): string {
    ...
    const fechaInicio =
      this.filter.fechaDesde?.replace(/-/g, "") ||
      new Date().toISOString().split("T")[0].replace(/-/g, "");
    const fechaFin =
      this.filter.fechaHasta?.replace(/-/g, "") ||
      new Date().toISOString().split("T")[0].replace(/-/g, "");
  ...
  private generateCSVForTurnosEspecialidadMedico(): string {
    ...
    const rows = this.reportData.turnosPorEspecialidadMedico.map(
      (item: any) => [
        item.medico,
        item.especialidad,
  ...
  private generateCSVForCancelacionesReprogramaciones(): string {
    ...
    const rows = this.reportData.cancelacionesReprogramaciones.map(
      (item: any) => [
        item.fecha,
        item.paciente,
        item.medico,
  ...
  private downloadFileFromContent(
    content: string,
    fileName: string,
    mimeType: string
  ): void {
  ...
  onTipoVistaChange(): void {
    ...
    if (this.isReportMode()) {
      ...
      if (this.tipoVista !== "busqueda") {
        ...
        console.log(
          "Modo reporte activado. Se requieren fechas para generar reportes."
        );
      }
    }
  ...
  private generateReportByTipoVista(): void {
    if (!this.validateRequiredFilters()) {
      alert(
        "Por favor complete los filtros obligatorios para generar el reporte"
      );
      return;
    }
  ...
  cargarSlotsDisponibles(staffMedicoId: number): void {
    ...
    this.agendaService
      .obtenerSlotsDisponiblesPorMedico(staffMedicoId, 4)
      .subscribe({
        next: (response: any) => {
          ...
          this.slotsDisponibles = slots.filter((slot: any) => {
            ...
            const currentDateTime = new Date(
              `${this.selectedTurno.fecha}T${this.selectedTurno.horaInicio}`
            );
  ...
  confirmarReagendamiento(): void {
    ...
    if (
      !this.motivoReagendamiento ||
      this.motivoReagendamiento.trim().length < 5
    ) {
      this.errorMessageReagendar =
        "Debe ingresar un motivo de al menos 5 caracteres para reagendar el turno";
      return;
    }
    ...
    this.turnoService
      .reagendar(this.selectedTurno.id!, reagendamientoData)
      .subscribe({
        next: (response) => {
          ...
          alert(
            `Turno reagendado exitosamente!\n\nNueva fecha: ${this.formatDate(
              this.slotSeleccionado!.fecha
            )}\nHorario: ${this.slotSeleccionado!.horaInicio} - ${
              this.slotSeleccionado!.horaFin
            }\nMédico: ${this.slotSeleccionado!.staffMedicoNombre} ${
              this.slotSeleccionado!.staffMedicoApellido
            }`
          );
